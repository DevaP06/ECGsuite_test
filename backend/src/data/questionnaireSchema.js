// Canonical clinical-history questionnaire question bank.
// Served by GET /api/questionnaire/:analysisId — mirrors the structure the
// frontend's QuestionGroup/Question types expect (see frontend types/questionnaire.ts).
// This is the source of truth; the frontend's local DEFAULT_QUESTIONNAIRE_SECTIONS
// is only a fallback for when this endpoint is unreachable.
...
export const VALID_QUESTION_IDS = new Set(
  QUESTIONNAIRE_SECTIONS.flatMap(section => section.questions.map(q => q.id))
);
