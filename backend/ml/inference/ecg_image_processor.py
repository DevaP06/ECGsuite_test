...
class ECGImageProcessor:
    ...
    def estimate_intervals(self, signal: np.ndarray, peaks: np.ndarray) -> Dict[str, float]:
        try:
            ...
            p_pre_win = int(0.20 * self.sampling_rate)
            t_post_win = int(0.40 * self.sampling_rate)
            min_t_delay = int(0.08 * self.sampling_rate)
            ...
            for p in peaks:
                ...
                p_end = max(0, p - int(0.06 * self.sampling_rate))
                if p_end > p_start:
